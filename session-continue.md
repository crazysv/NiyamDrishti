...
> **Usage:** Set this file up as a custom slash command in your IDE (see "Where to put this" below), then run `/session-continue` when resuming work — mid-task, after a context window reset, or after any gap where you're not 100% sure what the immediately-prior state was. If you're starting a genuinely new session with no prior context at all, use `/session-start` instead. If you suspect real drift (code not matching docs), use the fuller `docs/13_RECOVERY.md` protocol instead of this lighter checklist.
