...
class Settings(BaseSettings):
    ...
    # ------------------------------------------------------------------
    # OCR (PaddleOCR default, Tesseract fallback, Optional Gemini OCR-04)
    # ------------------------------------------------------------------
    PADDLEOCR_LANG: str = "en"
    OCR_MODEL_CACHE_DIR: str = "./ocr_models"
    OCR_PROVIDER: str = "auto"  # "auto" | "local" | "paddleocr" | "gemini" | "enhanced"
    OCR_ENGINE: str | None = None  # Alias support for Render environment settings
    GEMINI_API_KEY: str = ""
    GEMINI_API_KEY_2: str = ""
    GEMINI_API_KEY_3: str = ""
    GEMINI_API_KEYS: str = ""  # Comma-separated list of API keys for auto-rotation pool
    GEMINI_MODEL: str = "gemini-3.5-flash-lite"  # Demo-safe primary; stronger Flash models remain fallbacks
    ...
    def get_gemini_api_keys(self) -> list[str]:
        """
        Returns an ordered, deduplicated list of non-empty Gemini API keys for rotation pool.
        Parses GEMINI_API_KEYS (comma-separated), then appends GEMINI_API_KEY,
        ...
        if self.GEMINI_API_KEYS:
            for k in self.GEMINI_API_KEYS.split(","):
                ...
                if k_clean and k_clean not in keys:
                    keys.append(k_clean)
        for candidate in (self.GEMINI_API_KEY, self.GEMINI_API_KEY_2, self.GEMINI_API_KEY_3):
            c_clean = (candidate or "").strip()
            if c_clean and c_clean not in keys:
                keys.append(c_clean)
        return keys
    ...
    def sync_ocr_provider(self) -> "Settings":
        if self.OCR_ENGINE and self.OCR_PROVIDER == "auto":
            self.OCR_PROVIDER = self.OCR_ENGINE
        return self
