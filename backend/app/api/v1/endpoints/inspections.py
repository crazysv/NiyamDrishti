...
async def upload_inspection_image(
    ...
    if inspection.officer_id != current_user.id and current_user.role not in ["supervisor", "admin"]:
        if inspection.status in ["sync_pending", "draft", "syncing"] and current_user.role in [
            "officer",
            "field_officer",
        ]:
...
async def delete_inspection(
    ...
    """
    Deletes an inspection and its cascading child records (images, fields, violations, reports).
    """
    stmt = select(Inspection).where(Inspection.id == inspection_id)
    result = await db.execute(stmt)
    inspection = result.scalar_one_or_none()
    ...
    if inspection.officer_id != current_user.id and current_user.role not in ["supervisor", "admin"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You do not have permission to delete this inspection",
        )

    # Record deletion in immutable AuditLog
    ...
    # Delete child records in correct dependency order
    ...
    await db.execute(delete(Report).where(Report.inspection_id == inspection_id))
    await db.execute(delete(InspectionImage).where(InspectionImage.inspection_id == inspection_id))
    await db.execute(delete(Inspection).where(Inspection.id == inspection_id))
    await db.commit()

    return {
        "success": True,
        "id": str(inspection_id),
        "message": "Inspection and associated records deleted successfully",
    }
...
async def extract_inspection_declarations(
    inspection_id: uuid.UUID,
    ocr_provider: str | None = Query(
        default=None,
        description="Optional OCR engine: 'local', 'paddleocr', 'gemini', or 'auto'",
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
) -> list[ExtractedField]:
    ...
    Supports optional Gemini Vision OCR provider (OCR-04) alongside default PaddleOCR/Tesseract.
    """
    ...
    if ocr_provider and ocr_provider.lower().strip() not in {"local", "paddleocr", "gemini", "auto", "enhanced"}:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Unsupported OCR provider. Use local, paddleocr, gemini, auto, or enhanced.",
        )

    ocr_service = OCRService()
    if ocr_provider and ocr_provider.lower().strip() == "gemini" and not ocr_service.is_gemini_available():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Gemini OCR provider requested, but GEMINI_API_KEY is not configured on this server.",
        )
    ...
    for img in inspection.images:
        ...
        try:
            ...
            if img.calibration_scale_mm_per_px is None:
                try:
                    calib_service = OpticalCalibrationService()
                    # Barcode decoding and OpenCV image work are synchronous and
                    # CPU-bound.  Keep FastAPI's event loop free so Render can
                    # answer its five-second health probes during processing.
                    calib_res = await asyncio.to_thread(calib_service.calibrate_image, img_bytes)
                    if calib_res.is_calibrated and calib_res.scale_mm_per_px is not None:
            ...
            # OCR preprocessing and the synchronous Gemini SDK call must not
            # run on the async request loop; a blocked loop makes Render mark
            # an otherwise live instance unhealthy and restart it.
            ...
            record_ocr_duration(time.perf_counter() - t0_ocr, engine=ocr_res.engine_used, status="success")
            declarations = extraction_service.extract_from_ocr_result(ocr_res)
            all_declarations.extend(declarations)
            all_ocr_lines.extend(ocr_res.lines)
        except Exception as ocr_err:
            logger.error(f"OCR processing failed for image {img.id}: {ocr_err}", exc_info=True)
            fallback_engine_name = ocr_provider or settings.OCR_PROVIDER or "paddleocr"
            record_ocr_duration(0.0, engine=fallback_engine_name, status="error")
            ocr_failures.append(f"{img.image_role}: {ocr_err}")
        finally:
            del img_bytes
            import gc
    ...
    # Do not turn a provider outage into an apparently valid, empty inspection.
    # A partial multi-image result is useful to an officer, but a run in which
    # every OCR invocation failed must be retried by the offline queue.
    if processed_image_count == 0 and ocr_failures:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail={
                "message": "OCR could not process any inspection image.",
                "provider": ocr_provider or settings.OCR_PROVIDER,
                "failures": ocr_failures,
            },
        )

    # A declaration header and its value can be printed on separate package
    # panels. Re-run MRP extraction across the complete inspection evidence,
    # retaining the value panel as the bounding-box source. This replaces the
    # per-panel MRP candidates only; other per-image fields remain available
    # for multi-image consistency checks.
...
async def get_inspection_evidence(
    ...
    # A record is created before mobile photos are uploaded so an interrupted
    # upload can be retried idempotently. It is not evidence until a photo
    # exists: otherwise an empty response incorrectly looks compliant.
    if not inspection.images:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Evidence upload is still pending. Upload at least one inspection image before viewing results.",
        )
    if inspection.status in {"sync_pending", "syncing", "failed"}:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Evidence upload or analysis is still pending. Retry the upload before viewing results.",
        )
    ...
    failed_count = 0
    product_name = None
    product_name_candidates: list[tuple[float, str]] = []
    non_product_phrases = (
        "cut here",
        "stored refrigerated",
        "refrigerated below",
        ...
        if f.field_type == "commodity_name" and f.parsed_value:
            ...
            if isinstance(raw_cname, str) and raw_cname.strip().startswith("{"):
                try:
                    ...
                    pass
            normalized_cname = " ".join(str(raw_cname).split())
            ...
            if len(normalized_cname) >= 3 and not any(
                ...
                if front_img and f.source_image_id == front_img.id:
                    candidate_score += 0.1
                product_name_candidates.append((candidate_score, normalized_cname))
    ...
    if product_name_candidates:
        product_name = max(product_name_candidates, key=lambda candidate: candidate[0])[1]
