...
async def test_full_pipeline_capture_to_report(e2e_test_db):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        mock_ocr = OCRResult(
            ...
            lines=[
                OCRLine(
                    text="HIMALAYAN PREMIUM BASMATI RICE",
                    confidence=0.98,
                    bounding_box=BoundingBox(x=100, y=300, w=600, h=50),
                    source_image_id=str(img_data["id"]),
                ),
                OCRLine(
                    text="NET QUANTITY: 5 kg",
                    confidence=0.97,
                    bounding_box=BoundingBox(x=100, y=450, w=400, h=40),
                    source_image_id=str(img_data["id"]),
                ),
                OCRLine(
                    text="MRP Rs. 350.00 (INCL. OF ALL TAXES)",
                    confidence=0.99,
                    bounding_box=BoundingBox(x=100, y=600, w=650, h=40),
                    source_image_id=str(img_data["id"]),
                ),
                OCRLine(
                    text="MFD: 01/2026",
                    confidence=0.95,
                    bounding_box=BoundingBox(x=100, y=750, w=300, h=40),
                    source_image_id=str(img_data["id"]),
                ),
                OCRLine(
                    text="MANUFACTURED BY: Himalayan Foods Ltd, Delhi - 110001",
                    confidence=0.94,
                    bounding_box=BoundingBox(x=100, y=900, w=800, h=40),
                    source_image_id=str(img_data["id"]),
                ),
                OCRLine(
                    text="CONSUMER CARE: support@himalayan.in, 1800-111-222",
                    confidence=0.96,
                    bounding_box=BoundingBox(x=100, y=1050, w=750, h=40),
                    source_image_id=str(img_data["id"]),
                ),
                OCRLine(
                    text="COUNTRY OF ORIGIN: INDIA",
                    confidence=0.98,
                    bounding_box=BoundingBox(x=100, y=1200, w=450, h=40),
                    source_image_id=str(img_data["id"]),
                ),
            ],
        ...
        with patch("app.services.ocr.service.OCRService.process_image", return_value=mock_ocr):
            ...
            # A second run replaces both fields and violations.  This is the
            # normal retry/reprocess path after an improved image is captured.
            # It must not attempt to delete fields still referenced by an old
            # violation row.
            reprocess_resp = await ac.post(
                f"/api/v1/inspections/{inspection_id}/process",
                headers=headers,
            )
