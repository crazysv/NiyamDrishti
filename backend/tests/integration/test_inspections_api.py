...
async def test_inspection_create_and_upload_images(test_db):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        # A created record cannot be presented as a completed evidence result
        # until its first mobile photo reaches the server.
        pending_evidence_resp = await ac.get(
        ...
        # 8. Delete inspection: Officer 1 deletes their own inspection
        del_resp = await ac.delete(f"/api/v1/inspections/{inspection_id}", headers=headers)
        assert del_resp.status_code == 200
        del_data = del_resp.json()
        assert del_data["success"] is True

        # 9. Verify 404 after deletion
        get_after_del = await ac.get(f"/api/v1/inspections/{inspection_id}", headers=headers)
