...
export default function CaptureScreen() {
  ...
  const handleSaveAndQueue = async () => {
    ...
    try {
      const canProcessOnline = isOnline && navigator.onLine;
      await queueInspection(inspectionId, category, validImages, !canProcessOnline);
  ...
  return (
    <div className="flex flex-col w-full max-w-md mx-auto h-[100dvh] max-h-[100dvh] overflow-hidden bg-[#F9F7F2] text-[#1A1C1E] shadow-2xl relative select-none">
      ...
      {/* Compact category control — connection/sync state already appears above. */}
      <div className="px-4 py-1.5 flex justify-end items-center bg-[#F9F7F2] border-b border-[#D1CDC2] text-xs shrink-0">
        {/* Category Selector */}
        <select
          value={category}
          ...
          {CAPTURE_SLOTS.map((slot) => {
            ...
            return (
              <button
                ...
                className={`relative shrink-0 w-[68px] aspect-square rounded-sm overflow-hidden flex flex-col items-center justify-center transition-all ${
                  isActive
      ...
      {/* Controls Bar */}
      <div className="bg-[#F9F7F2] px-6 py-2.5 flex items-center justify-between border-t border-[#D1CDC2] shrink-0">
        {/* Flash / Torch Toggle */}
        <button
          onClick={toggleTorch}
      ...
      {/* Bottom Navigation */}
      <nav className="shrink-0 z-40 bg-[#F9F7F2]/90 backdrop-blur-md border-t border-[#D1CDC2] px-6 py-2 flex items-center justify-around">
