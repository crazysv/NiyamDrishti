...
export default function HistoryScreen() {
  ...
  const { pendingCount, syncNow, syncInspectionNow, isSyncing, refreshQueueState } = useOfflineQueue();
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      // 1. Remove from local IndexedDB if present
      try {
        const localMatches = await db.inspections
          .filter((rec) => rec.id === id || rec.backendId === id)
          .toArray();
        for (const match of localMatches) {
          await discardOfflineInspection(match.id);
        }
        await discardOfflineInspection(id);
      } catch (localErr) {
        console.warn("Local DB cleanup error:", localErr);
      }

      // 2. If it's a backend UUID or device is online, attempt server deletion
      const isRemoteUuid = !id.startsWith("insp_");
      if (isOnline && isRemoteUuid) {
        try {
          await deleteInspection(id);
        } catch (serverErr) {
          console.warn("Server delete warning:", serverErr);
        }
      }
      ...
      const msg = err instanceof Error ? err.message : "Failed to delete inspection";
      setErrorMessage(msg);
    } finally {
      setDeletingId(null);
    }
  ...
  const handleEvidenceAction = async (inspection: InspectionSummary) => {
    ...
    if (!isOnline) {
      setErrorMessage("This inspection still has photos waiting to upload. Connect to the internet, then retry the upload.");
      return;
    }

    const localInspection = await db.inspections
      .filter((record) => record.id === inspection.id || record.backendId === inspection.id)
      .first();
    if (!localInspection) {
      setErrorMessage("This pending server record has no photo retained on this device. Delete it and capture the package again.");
      return;
    }

    setRetryingId(inspection.id);
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-[#F0EDE5]">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            aria-label={`Delete inspection ${insp.id}`}
                            className="p-1.5 text-[#BA1A1A] hover:text-[#93000a] hover:bg-[#FFDAD6]/60 rounded-md transition-colors disabled:opacity-40"
                          >
                            {deletingId === insp.id ? (
