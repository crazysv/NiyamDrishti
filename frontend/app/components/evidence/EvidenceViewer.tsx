...
export default function EvidenceViewer({
  ...
  const imagePanels = React.useMemo(() => {
    ...
    evidence.image_panels?.forEach((panel) => {
      panelMap.set(panel.id, { ...panel, count: 0 });
    });
  ...
  const activeImageDimensions = imageDimensions[activeImageId] || evidence.primary_image_dimensions || null;
  const displayedImageRect = React.useMemo(() => {
    if (!activeImageDimensions || !viewportSize.width || !viewportSize.height) return null;
    const scale = Math.min(
      viewportSize.width / activeImageDimensions.width,
      viewportSize.height / activeImageDimensions.height,
    );
    const width = activeImageDimensions.width * scale;
    const height = activeImageDimensions.height * scale;
    return {
  ...
  }, [activeImageDimensions, viewportSize]);
  ...
  const handleItemClick = (item: EvidenceItem) => {
    setExpandedItemId((current) => current === item.item_id ? null : item.item_id);
    focusOnItem(item);
  };
    ...
    <div className="flex flex-col w-full bg-[#f9f9fc] text-[#1a1c1e] min-h-screen">
      ...
      <main className="max-w-4xl mx-auto w-full pb-28">
                  ...
                  onLoad={(event) => {
                    ...
                    if (naturalWidth && naturalHeight) {
                      setImageDimensions((current) => ({
                        ...current,
                        [activeImageId]: { width: naturalWidth, height: naturalHeight },
                      }));
                    }
                  }}
              ...
              >
                {evidence.items
                  ...
                  .map((item) => {
                    ...
                    const bbox = item.bounding_box;
                    const hasReliableBox =
                      bbox.width_pct > 0.25 &&
                      bbox.height_pct > 0.25 &&
                      bbox.width_pct <= 75 &&
                      bbox.height_pct <= 20 &&
                      bbox.left_pct >= 0 &&
                      bbox.top_pct >= 0 &&
                      bbox.left_pct + bbox.width_pct <= 100 &&
                      bbox.top_pct + bbox.height_pct <= 100;

                    if (!hasReliableBox) return null;
        ...
        <section className="px-4 py-4 bg-[#f9f9fc] border-b border-[#e2e2e5]">
          ...
          <p className="mt-2 text-[10px] text-[#545f72]">
            Pass = declaration found and accepted · Review = officer confirmation needed · Fail = confirmed rule breach
          </p>
        </section>
        ...
        <section className="flex flex-col bg-white divide-y divide-[#eeeef0] border-b border-[#e2e2e5]">
          {evidence.items.map((item) => {
            ...
            const sourcePanelLabel = imagePanels.find((panel) => panel.id === item.source_image_id)?.label;
            ...
            return (
              ...
              >
                <div className="flex flex-col gap-1 flex-1 min-w-0 pr-4">
                  <div className="flex items-center gap-2">
                    ...
                    {item.source_image_id && (
                      <span className="text-[9px] font-mono font-medium px-1.5 py-0.5 rounded bg-[#e8eef6] text-[#2c3e50] uppercase border border-[#c4d6eb]">
                        {sourcePanelLabel || (item.source_image_id.includes("front") || item.source_image_id.includes("pdp")
                          ? "Front PDP"
                  ...
                  {isExpanded && (
                    <div className="ml-9 mt-2 p-2.5 rounded border border-[#d6d6da] bg-[#f9f9fc] text-[11px] leading-relaxed text-[#44474c]">
                      <p className="font-mono font-semibold text-[#333e50]">OCR TEXT</p>
                      <p className="mt-0.5 break-words">{item.raw_text || "No readable declaration was found."}</p>
                      {item.verdict === "needs_review" && (
                        <p className="mt-2 text-amber-800"><strong>Why Review:</strong> {item.review_reason || "This declaration needs officer confirmation before a final compliance decision."}</p>
                      )}
                      {item.verdict === "fail" && (
                        <p className="mt-2 text-red-700"><strong>Why Fail:</strong> A rule engine check found a confirmed non-compliance. Review the cited finding above.</p>
                      )}
                    </div>
                  )}
                </div>
