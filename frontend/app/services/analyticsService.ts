...
export async function fetchAnalyticsSummary(token?: string): Promise<AnalyticsSummary> {
  ...
  const res = await fetch(`${API_BASE}/analytics/summary`, { headers, cache: "no-store" });
  if (!res.ok) {
...
export async function fetchViolationHotspots(
  ...
  const res = await fetch(`${API_BASE}/analytics/violation-hotspots?limit=${limit}`, { headers, cache: "no-store" });
  if (!res.ok) {
    const errorText = await res.text();
...
export async function fetchOfficerThroughput(token?: string): Promise<OfficerThroughput> {
  ...
  const res = await fetch(`${API_BASE}/analytics/officer-throughput`, { headers, cache: "no-store" });
  if (!res.ok) {
    const errorText = await res.text();
